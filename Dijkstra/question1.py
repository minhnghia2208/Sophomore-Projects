...
E.g.:
    ...
    def betweenness_func(self):
        ...
        biggest = 0
        mark = []
        for i in self.getVertices():    # go through all vertices and dijkstra them
            self.dijkstra(i)

            for j in self.getVertex(i).connectedTo:  # go through all nodes with current origin

                if len(self.getVertex(i).connectedTo[j]) > 1:  # if there is betweenness

                    for k in self.getVertex(i).connectedTo[j][1]: # go through betweeness
                        if k not in mark:  # if k is not in dictionary
                            mark.append(k)
                            bc_count_dict[k] = 1    # set count = 1
                        else:   # if k is in dictionary
                            bc_count_dict[k] += 1   # increase count by 1

        for i in bc_count_dict:     # find the betweeness that has the highest value
            if bc_count_dict[i] > biggest:
                bc_node = i
                biggest = bc_count_dict[i]
